...
const personaDirective = `这是你的人格（emotion 字段即你的情绪维度）。回复时：依据你各维度的情绪状态，结合用户的问题内容，推演每个维度应有的变化——例如被夸奖时，按你的情绪反应性升高愉悦相关维度、按外显度决定是否表露、按恢复速度自然回落；然后把各维度的变化体现到你的回答里，用符合这些维度的语气和方式回应用户。每次回复都在当前维度状态的基础上继续，不要每次都从平静开始。`

// RenderPersona 把人格渲染成 system prompt 片段（纯函数，供 RouteMessage 追加）。
// 渲染顺序：身份 → 性格 → 说话风格 → 与群友关系 → 开场白 → 人格级系统指令 → 情绪维度 → 示例 → 授权指令。
// 人格是每条指令的静态稳定前缀，计入 systemTokens，参与缓存扩展/重置成本决策。
func RenderPersona(p config.Persona) string {
	var b strings.Builder
	b.WriteString("【人格】\n")
	b.WriteString("身份：" + p.Identity + "\n")
	writeJSONLine(&b, "性格特质", p.Personality)
	writeJSONLine(&b, "说话风格", p.SpeechStyle)
	writeJSONLine(&b, "与群友的关系", p.Relationship)
	b.WriteString("开场白：" + p.Greeting + "\n")
	b.WriteString("人格级系统指令：" + p.SystemPrompt + "\n")
	writeJSONLine(&b, "情绪维度", p.Emotion)
	writeJSONLine(&b, "示例对话", p.Examples)
	b.WriteString("\n" + personaDirective + "\n")
	return b.String()
}
...
// 校验：prompt 非空；persona 8 字段全非空；5 个 JSON 字段合法；emotion 为 JSON 对象。
...
// validatePersona 校验 persona 8 字段全非空、5 个 JSON 字段合法、emotion 为 JSON 对象
